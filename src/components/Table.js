...
import {getVOO, formatterDate} from 'utils'
...
export default class Table extends Component {
    ...
    if (loadMore) {
      this.tableBody.onscroll = function() {
        if (!_this.state.scroll) {
          ...
          // _this.tableBody.onresize = ()=>{
          //   console.log("adfa");
          // }
          // window.aa = _this.tableBody
        }
  ...
  updateHeaderWidth = (_this) => {
    if (_this.props.header && _this.props.loadMore) {
      ...
      for (var i = 0; i < _this.props.header.length; i++) {
        if (_this['header' + i]) {
          _this["scrollheader" + i].style.width = _this['header' + i].clientWidth + "px"; //设置title的每个td宽，
        }
      }
      _this.scrollheader.style.width = _this.header.offsetWidth + "px";
    }
  }
  ...
  bodyTR = (rowData, row) => {
    ...
    let className = classname(
      ...
      ? styles.canClick
      : "",
    rowData.id == selectValue
      ? styles.select
      : "");
    rowClick = rowClick || new Function();
    return (<tr key={row} className={className}>
      {
        header && header.map((data, column) => {
          ...
          value = data.propsKey == "date"
            ? formatterDate(value)
            : value; // lambo 偷懒
          value = this.parseValue(value, data, itemData);
          itemData.value = value;
          let view = this.parseView(value, data, itemData);
          return (<td key={column} style={data.style} data-d={itemData.column} className={data.className} onClick={() => rowClick(itemData)}>{view}</td>);
        })
      }
    </tr>);
  }
    ...
    return (<tr>
      {
        header && header.map((data, index) => {
          return (<td ref={this.bindRef.bind(this, (scrollHeader ? "scroll" : "") + "header" + index)} key={index}>{data.name}</td>);
        })
      }
    </tr>);
  }
  ...
  render() {
    ...
    return (<div style={style} className={classname(styles.table, className)}>
      <div className={styles.body} ref={this.bindRef.bind(this, "tableBody")}>
        <table ref={this.bindRef.bind(this, "header")}>
          <thead>
            {this.headerTR(0)}
          </thead>
          <tbody>
        ...
        {fetching && this.bottomView("正在加载更多...")}
        {
          !fetching && (
            ...
            : noMore != undefined && this.bottomView(<span className="pointer" onClick={loadMore}>查看更多</span>))
        }
      </div>
      <table ref={this.bindRef.bind(this, "scrollheader")}
